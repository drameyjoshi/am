...
\end{prop}
\begin{proof}
$x < y \Rightarrow y - x > 0$. If $x = \overline{\seq{a_n}}, y = 
\overline{\seq{b_n}}, y - x = \overline{\seq{c_n}}$, where $c_n = b_n -
a_n$ then $y - x > 0$ implies that there is a sequence $\seq{d_n} \in
\overline{\seq{c_n}}$ that is positively bounded away from zero. That is,
$d_n > k$ for a rational number $k > 0$. Further, $\seq{d_n} \sim 
\seq{c_n}$. Therefore, given an $\epsilon > 0$, we can find $N \in \son$
such that for all $n \ge N$, $|d_n - c_n| < \epsilon \Rightarrow c_n
- \epsilon < d_n < c_n + \epsilon \Rightarrow k < d_n < c_n + \epsilon
\Rightarrow k < b_n - a_n + \epsilon \Rightarrow a_n + k < b_n + \epsilon
\Rightarrow a_n < a_n + k < b_n + \epsilon$ for all $n > N$.
\end{proof}
