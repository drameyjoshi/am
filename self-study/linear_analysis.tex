...
x\rVert \rightarrow d$.
\end{prop}
\begin{proof}
Since $d = \inf_{m \in M}$, there exists a small enough $\epsilon > 0$ for 
which there is a point $m_1 \in M$ such that $\lVert m_1, x\rVert = d+\epsilon$.
Choose $m_2 \in M$ such that $\lVert m_2, x\rVert = d + \epsilon/2$ and in 
general, $m_n$ such that $\lVert m_n, x\rVert = d + \epsilon/2^{n-1}$.
\end{proof}

\begin{prop}\label{s4p5}
The sequence $\{m_n\}$ of points constructed in \ref{s4p4} is Cauchy.
\end{prop}
\begin{proof}
...
+ \lVert x - (1/2)(m_n + m_k)\rVert^2 = 2\lVert x - m_n\rVert^2 + 2\lVert x -
m_k\rVert^2$.
\end{proof}
...
\end{thm}
\begin{proof}
We will first show that for a given $x \in H$, there exists a unique $y \in M$
such that $\lVert x - y\rVert$ is the least. That is, if $d = \inf_{m \in M}
\lVert x - m\rVert$ then there exists $y \in M$ such that $\lVert x - y\rVert
= d$.
