...
If $T: H \mapsto \mathbb{C}$ be a bounded linear operator. Then $T$ is 
continuous.
\end{prop}
\begin{proof}
...
= x - y - \alpha n = z - \alpha n$ so that $\lVert z^\prime \rVert = \lVert z
...
\begin{thm}[Reisz representation theorem]\label{s4t2}
...
\[
\sup_{\lVert x \ne \rVert}\frac{|L(x)|}{\lVert x \rVert} = \lVert w \rVert
\]
