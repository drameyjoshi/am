...
Then $\sum a_n$ converges if $L < 1$, diverges if $L > 1$ and is inconsludive
if $L = 1$.
\end{prop}
\begin{proof}
...
\]
If the integral is finite, $\sum_{n \ge 2}$ is finite and hence $a_1 + \sum_{n
\ge 2} = \sum_{n \ge 1}a_n$ is finite. That is $\sum a_n$ converges.
\end{proof}
...
\section{Sequences and series of functions}\label{s6}
...
\[
\lim_{n \rightarrow \infty} ||f_n - f||_A = 0.
\]
...
is also evident that $||f_n - f|| = 1$.

\begin{thm}[Uniform Limit Theorem]\label{s6t1}
...
\begin{defn}[Cauchy Criterion]\label{s6d5}
...
$||f_n - f_m||_A < \epsilon$ then $\{f_n\}$ converges uniformly.
\end{defn}

We will use the Cauchy criterion to prove,
\begin{thm}[The Weierstrass M-test]\label{s6t2}
