...
floor_id = [1, 2, 1, 2, 1, 2, 1, 2, 1]
floor_sort = [0, 1, 0, 1, 0, 1, 0, 1, 0]
...
for i in range(len(tour_allocs)):
    solver.Add(tour_allocs[i] <= ubound)
    solver.Add(tour_allocs[i] >= lbound)
...
solver.Maximize(sum(obj_list))
#solver.ExportToFile('testPriorityMessByCodeSharing.txt')

status = solver.Solve()

if status == pywraplp.Solver.FEASIBLE or status == pywraplp.Solver.OPTIMAL:
    print(f'Total time = {solver.Objective().Value()}')
    for t in range(n_tours):
        print(f'Cleaning tour {t} will clean:')
        for c in range(n_cleaning_areas):
            if x[(t, c)].solution_value() > 0.5:
